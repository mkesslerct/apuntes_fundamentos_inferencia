...
sobre la función de densidad, la función nucleo y el ancho de banda:
...
-   El núcleo es una función de densidad simétrica y acotada, con
    ...
    valor del momneto de orden 2 asociado a la densidad $K$.
...
si $\sigma$ es la desvación típica de la variable de partida la podemos
aproximar mediante la cuasidesvación típica muestral, $S$, y podemos
aproximar el valor de $h_n$ mediante
$$\hat h_n=\left( \frac{8 \pi^{1/2}R(K)}{3\mu_2^2(K)n} \right)^{\frac{1}{5}}S.$$

Si adicionalmente cosnideramos el núcleo normal, lo anterior queda como
...
#valor del ancho de banda con estimacion no parametrica de R(f")
